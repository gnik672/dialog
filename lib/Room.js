...
class Room extends EventEmitter
{
	...
	async _handleProtooRequest(peer, request, accept, reject)
	{
		...
		switch (request.method)
		{
			...
			case 'transcriptionRequest':
				{
					...
					const targetPeerId = request.data.to;					

					if (this._protooRoom.hasPeer(targetPeerId)) {
						const targetPeer = this._protooRoom.getPeer(targetPeerId);
						await targetPeer.notify("newTranscSub", {from: peer.id});
						accept();
					}
					else{
						reject();
			...
			case 'stopTranscriptionRequest':
				{
					...
					if (this._protooRoom.hasPeer(targetPeerId)) {
						const targetPeer = this._protooRoom.getPeer(targetPeerId);
						await targetPeer.notify("removeTranscSub", {from: peer.id});
						accept();
					}
					else{
						reject();
			...
			case 'sendTranscription':
				{
					...
					try{	
						for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })){
							await otherPeer.notify("transcAvailable", {message: message, language: language, from:peer.id});
						}
						accept();
						break;
			...
			case "sendPresenterInfo":
				{
					const {presenterId} = request.data;
					console.log(presenterId);
					for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })){
						await otherPeer.notify("presenterInfo", {presenter: presenterId})
					}
					accept();
					break;
				}
			...
			case "sendSpeakerInfo": {
					const { speakerId } = request.data;
					console.log(`🔊 New speaker: ${speakerId}`);
				  
					for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
					  await otherPeer.notify("speakerInfo", { speaker: speakerId });
					}
			...
			case "sendQuestionRequest":
				{
					const {raiseHand} = request.data;
					console.log(`received hand raise request with value: ${raiseHand}`);
					for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })){
						await otherPeer.notify("questionRequest", {raiseHand: raiseHand, from:peer.id })
					}
					accept();
					break;
				}		
			
			case "sendQuestionRequestReply":
				{

					const {to: targetPeerId, accept :decision} = request.data;					
					console.log(request.data)

					if (this._protooRoom.hasPeer(targetPeerId)) {
						const targetPeer = this._protooRoom.getPeer(targetPeerId);
						console.log(`recieved hand raised proccess from pressenter. Presenter ${accept?"accepts" :"denies"} the request`)
						await targetPeer.notify("questionRequestReply", {from: peer.id, accept: decision});
						accept();
					}
					else{
						reject();
			...
			case "askPresenterInfo":
				{
					
					for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })){
						await otherPeer.request("givePresenterInfo", {from: peer.id})
					}
					accept();
					break;
				}
